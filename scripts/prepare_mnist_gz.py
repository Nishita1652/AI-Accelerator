...
def load_raw_mnist():
    if not (os.path.exists(TRAIN_IMAGES_PATH) and os.path.exists(TRAIN_LABELS_PATH)):
        return None, None
    try:
        print("[INFO] Raw MNIST .gz files detected. Unpacking...")
...
def generate_synthetic_data():
    print("[WARNING] Raw dataset missing/corrupt. Generating synthetic fallback...")
    np.random.seed(42)
...
print(f"[SUCCESS] Dataset ready at '{OUTPUT_FILE}'")